...
- Mount root 是 endpoint 选择依据；mount-aware 文件、PTY、Agent 和 worktree 操作仍必须通过 approved root guard。
- Agent/Terminal 和 Space Explorer 的执行/文件目录由 Space 的 `boundary` / `directoryPath` 决定；当项目配置固定 worktree 根目录时，这些目录可以在 mount root 外，但必须仍位于同一 endpoint 的 approved root 内。
...
2. 一个 Space 的 mount-aware 文件/PTY/worktree 操作必须通过 `targetMountId` 解析 endpoint。
   路径 authority 来自 endpoint 上的 approved roots；固定 worktree root 可以在 mount root 外。
